...
ks.stopDownloadTexture = function () {
    isStopDownloadTexture = true;
};
ks.starDownloadTexture = function () {
    isStopDownloadTexture = false;
    while (cachedDownloadTask.length > 0) {
        const task = cachedDownloadTask.shift();
        if (task) {
            mod.KSDownloadTexture(task.path, task.width, task.height, task.callback, task.limitType);
        }
    }
};
const mod = {
    ...
    KSDownloadTexture(path, width, height, callback, limitType = false) {
...
export default {
    KSDownloadTexture: mod.KSDownloadTexture,
};
